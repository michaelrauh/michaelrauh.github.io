...
Logic interferes with itself. Adding new logic can impact the state of a variable at a point in time. It can impact which lines of code run at a given time. It can also introduce instances where the concrete type of an interface is now what would be expected.
...
1. If the branching occurs too deeply and you don't want to commit, but you want to get some of the benefit, insert the strategy pattern. Branching occurs for one of three reasons.
  ...
  1. You used to have clear code separation but now your code paths have collapsed (you have two different calls to the same function somewhere up the call chain)
